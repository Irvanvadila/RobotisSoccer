...
if __name__ == '__main__':
    try:
        ...
        # Pole Tracking
        polePosSetpoint = 0.02
        poleDistStop = 10000
        poleDistComp = 10
        polePosFilter.setKf(25)
        poleSizeFilter.setKf(25)
        polePosPID.setPoint(0)
        #default 10.0/100, 0.01/100, 0.0/100
        polePosPID.setPID(11.0/100, 0.00/100, 0.0/100) #KP #KI #KD NORMALISASI = PER 100
        # Base Tracking
        basePosFilter.setKf(100)
        baseSizeFilter.setKf(25)
        basePosPID.setPoint(0.0)
        basePosPID.setPID(0.00, 0.0, 0.0)
        errComp = 80

        stepLimit = 0.010
        stepSpeed_x = 0.053 #0.055
        stepPeriod = 0.64 #0.64
        headPan = 0
        headTiltForward = -0.20
        headTiltReturn = -0.3
        headTilt = headTiltForward

        yawRotationOffset = -400
        yawLimit = 200
        forwardTime = 3400
        rotateTime = 6750
        rotateStep = 0.045
        ...
        while not rospy.is_shutdown():
            yawError = gyroYawIntegrator - yawOffset
            ...
            if systemState == 'PRE_RUN':
                ...
                LED_Status(0, 1, 0)
                Motion_WalkingParams(stepSpeed_x / 2, 0, 0.0, stepPeriod)
                Motion_Start()
                yawOffset = gyroYawIntegrator
                yawError = gyroYawIntegrator - yawOffset
                ...
                if taskSequence == 'FORWARD':
                    ...
                    t += 1
                    xSpeed = stepSpeed_x
                   # Motion_WalkingParams(xSpeed, 0.0, yawPID.u, stepPeriod)
                    Motion_WalkingParams(xSpeed, limit(polePosPID.u, -stepLimit, stepLimit), yawPID.u, stepPeriod)
                #    if Ticks() - taskTimer >= 7500:
                 #       systemState = 'PRE_STOP'
                   #     taskSequence = 'ROTATE'
                elif taskSequence == 'ROTATE':
                    if Ticks() - taskTimer < forwardTime:
                        Motion_WalkingParams(stepSpeed_x, 0.000, yawPID.u, stepPeriod)
                    elif Ticks() - taskTimer < rotateTime:
                        # Rotation
                        Motion_WalkingParams(rotateStep, 0.00, rotateYaw, rotatePeriod) #default 0.040, 0.00, -950, 0.62
                    else:
                        yawOffset += yawRotationOffset #default -420
                        yawError = gyroYawIntegrator - yawOffset
                        yawPID.reset()
                        basePos.x = 0
                    ...
                    # xSpeed = xSpeed / (1 + abs(yawError / errComp / 100))
                    xSpeed = stepSpeed_x
                    Motion_WalkingParams(xSpeed, limit(basePosPID.u, -stepLimit, stepLimit), limit(yawPID.u, -yawLimit, yawLimit), stepPeriod)
                    #Motion_WalkingParams(xSpeed, 0, limit(yawPID.u, -yawLimit, yawLimit), stepPeriod)
                    if baseSizeFilter.out > baseSizeStop:
                        LED_RGB(0, 0, 31)
                        taskTimer = Ticks()
                        BuzzerTone(1, 500)
                ...
                LED_RGB(0,0,0)
                LED_Status(0,0,1)
                Motion_WalkingParams(0, 0, 0, stepPeriod)
                Motion_InitWalking()
                Motion_InitHead()
                Buzzer(2000)
                headTilt = headTiltForward
