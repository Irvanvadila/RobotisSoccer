...
def Init():
    ...
    rospy.Subscriber("/KRSBI/processing_image/deteksi_bola/image_processing/state", String, ballState_Callback)
    rospy.Subscriber("/KRSBI/processing_image/deteksi_bola/coordinate/bola", Vector3, ballCoord_Callback)
        ...
        while not rospy.is_shutdown():
            ...
            if systemState == 'PRE_RUN':
                ...
                if taskSequence == 'FORWARD':
                    ...
                    xError /= 5
                    if xError > 0.020: xError = 0.020
                    elif xError < -0.020: xError = -0.020
                    if ballState == "OK":
                        Motion_WalkingParams(xSpeed, xError, u, stepPeriod)
                        LED_RGB(0, 31, 0)
                    else:
                        Motion_WalkingParams(-0.010, xError, u, stepPeriod)
                        LED_RGB(0, 0, 0)
                    # if ballSizeFiltered > 40:
                    #     LED_RGB(0, 0, 31)
                    #     Motion_Stop()
                    #     Motion_InitAction()
                    #     Motion_ActionNum(action_rightKick)
                    #     Motion_InitWalking()
                    #     Motion_WalkingParams(0.00, 0, u, 0.45)
                    #     Motion_Start()
                    #     BuzzerTone(1, 500)
                    #     taskTimer = Ticks()
                    #     #taskSequence = 'PRE_STOP'
