    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            if systemState == 'PRE_RUN':
                ...
                if taskSequence == 'JALAN' :
                    if Ticks() - taskTimer > 2000:
                        ...
                        play_dtmf_tone(stream, digits, length=3.0)
                        stream.close()
                        p.terminate()
                        taskSequence = 'LISTEN'
                    else:
                ...
                elif taskSequence == 'LISTEN':
                    # start Recording
                    rospy.loginfo("start listen")
                    ...
                    for char, freqCocok in DTMF_TABLE.items():
                        if (select_pjg(filter_freq, freqCocok[0]) and
                            ...
                            if (char == '1'):
                                taskTimer = Ticks()
                                rospy.loginfo("open cam")
                                taskSequence = 'BOLA'
                    Motion_WalkingParams(stepSpeed_x, 0.00, u, stepPeriod)

                if taskSequence == 'BOLA':
                    ...
                    if ballState == "OK":
                        ...
                        head = (-1 * (ballSizeFiltered / 100)) + -0.2
                        if head > 0:
                            head = -0.4
                        elif head < -1.2:
                            head = -1.2
                        ...
                        LED_RGB(0, 31, 0)
                        if ballSizeFiltered >50 :
                            rospy.loginfo('tendang')
                            LED_RGB(0, 0, 31)
                            time.sleep(0.2)
                            Motion_Stop()
                            Motion_InitAction()
                            Motion_ActionNum(action_sideKickRight)
                            Motion_InitWalking()
                            Buzzer(3000)
                            time.sleep(0.2)
                            Buzzer(2000)
                        ...
                        Motion_WalkingParams(0.0, xError, u, stepPeriod)
                        LED_RGB(0, 0, 0)
                        rospy.loginfo("gaada bola")
